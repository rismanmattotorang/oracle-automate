---
name: oracle.skill.clean_core_audit
...
1. **Inventory** — call `oracle.oic.get_project_contents` on `{{package}}`. For each member, note its kind (program, class, interface, CDS view, function module, include).
...
3. **Where-used cross-check** — for each non-released-API touchpoint, call `oracle.oic.where_used` to see whether the dependency is limited to the audited package or leaks into others.
4. **Clean Core procedure** — call `oracle.docs.search` with `"Clean Core extension framework released API"` to retrieve the canonical procedure.
...
Do NOT propose code changes; produce only the audit report. Code changes are a separate skill (oracle.skill.released_api_migration, planned).
