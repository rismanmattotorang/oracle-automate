# Deploying Oracle-Automate on Kubernetes

These manifests deploy the **read-only** Oracle-Automate server tier.
...
| File | Purpose |
|---|---|
| `namespace.yaml`            | Creates the `oracle-automate` namespace. |
...
```bash
# From the repo root
docker build -t ghcr.io/acme/oracle-automate:$(git rev-parse --short HEAD) -f deploy/Dockerfile .
docker push  ghcr.io/acme/oracle-automate:$(git rev-parse --short HEAD)
```
...
- **HashiCorp Vault**: register `oracle-automate-secrets` as a Vault path,
...
- **External Secrets Operator**: declare a `SecretStore` against your
  ...
  and an `ExternalSecret` that resolves `oracle-automate-secrets`.
...
mechanism that creates a `Secret` named `oracle-automate-secrets` works.
...
```bash
kustomize edit set image ghcr.io/acme/oracle-automate=ghcr.io/acme/oracle-automate:NEW_TAG
kubectl apply -k deploy/k8s/
kubectl rollout status deploy/oracle-automate-server -n oracle-automate
```
...
```bash
kubectl port-forward -n oracle-automate svc/oracle-automate-server 3030:3030
...
```bash
kubectl rollout restart deploy/oracle-automate-server -n oracle-automate
```

### Tighten the read-only-ness

The Deployment intentionally does NOT pass `--enable-writes`.  Writes
are routed through the separate `oracle-automate-gw` Deployment that lives
in a dedicated namespace.  To verify the server is read-only:

```bash
kubectl logs -l app.kubernetes.io/name=oracle-automate-server -n oracle-automate \
  | grep "read_only=true"
```
