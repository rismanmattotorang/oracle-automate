...
Phase 2 of Oracle-Automate is informed by source-reading three production
...
| **SAP Business Accelerator Hub** | api.sap.com | Official SAP sandbox for hundreds of S/4HANA Cloud OData v2/v4 APIs; free `APIKey` from a SAP Community login; primary "live backend" path for Oracle-Automate v1.3+ |
...
- **Design move**: `oracle_automate_kb::doc_tree::DocumentTree` builds
...
- **Design move**: three small pure-Rust modules in
  `oracle_automate_ingest` — `robots::RobotsTxt` (RFC 9309 subset),
...
- **Insight for us**: the *form* of a behavioural skill — frontmatter +
  ...
  Oracle-Automate's existing `SkillRegistry` without any change to the
...
## Where Oracle-Automate now improves on the references

| Concern | Reference behaviour | Oracle-Automate (Phase 2 + ADT) |
|---|---|---|
...
| **AGENTS.md** | mdk-mcp pattern | **Loaded from disk** at `./AGENTS.md` or `./.oracle-automate/AGENTS.md`; surfaced in `initialize.instructions` and as `agents://guardrails` resource |
...
| **Knowledge integration** | none (RFC server: opaque) | **First-class RAG layer**: `sap.docs.search` returns cited snippets across Help Portal, ABAP, BPMN, LeanIX, all from `oracle-automate-rag` (Phase 1A) |
...
| **RFC metadata cache** | thupalo: file-based, persistent, compressed; ~1–5 ms cached vs ~200–500 ms direct | **`MetadataCache<C: SapClient>` decorator** in `oracle-automate-rfc`: TTL-keyed `(function, language)` cache over any `SapClient`. Bulk reads split into hits + misses so only misses hit the SAP system. `CacheStats` surfaced for Prometheus. `invalidate_all()` for system-role flips. No extra deps (`tokio::sync::RwLock`). **Wired through `apps/oracle-automate-server` with `--metadata-cache-ttl-secs` (default 300)**, exposed as `sap-cache://stats` resource + `sap.system.cache_stats` / `sap.system.cache_invalidate` MCP tools (thupalo's `get_metadata_cache_stats` convergence), surfaced live in the TUI KB tab and the web Operations page. 3 server-binary integration tests verify the hit counter moves on repeat calls. Persistence + compression deferred behind the trait. |
| **Skill-aware gateway routing** | marianfoo registry: "agents invoke skills, not raw tools" | **`apps/oracle-automate-gw::match_skill()`** maps user-intent keywords to `sap.skill.*` prompts and invokes them via `prompts/get` before falling back to raw tool calls. 8 intents routed: SoD audit, BW migration, period close, ABAP code review, OData design, transport impact, Clean Core audit, Karpathy guidelines. Keyword router today; LLM-driven router in v1.5 without changing the channel/tool trait surface. |
| **Hierarchical document tree** | OpenKB + PageIndex: tree-of-contents data structure + LLM tree search | **`oracle_automate_kb::DocumentTree`** + `KnowledgeStore::get_document_tree` default impl + `sap.kb.navigate` MCP tool. Pure-Rust tree builder from Markdown ATX / numbered / `SECTION:` markers. Each node carries title, extractive summary, byte range, approx token count, children. Navigation reasoning stays with the agent (the MCP client), keeping the crate LLM-free. 6 unit tests + 4 in-process binary integration tests. |
...
| **Crawler — robots.txt** | Crawl4AI yes; absent in MCP reference projects | **`oracle_automate_ingest::robots::RobotsTxt`** — RFC 9309 subset (User-agent groups, longest-prefix Allow/Disallow, Crawl-delay). 7 unit tests cover wildcard, specific-agent override, longest-prefix-allow-wins, comment stripping, empty body. |
| **Crawler — rate limiting** | Crawl4AI yes | **`oracle_automate_ingest::rate_limit::RateLimiter`** — per-host token-bucket spacing; `set_interval` per host (e.g. from a parsed `Crawl-delay:`). 5 unit tests including independent-bucket and post-set-interval cadence. |
| **Crawler — content filter** | Crawl4AI: "fit markdown" (BM25 against page topic) | **`oracle_automate_ingest::fit_markdown::fit_markdown_filter`** — pure-Rust BM25 block filter. Drops short nav / breadcrumb blocks unconditionally, always keeps long substantive blocks, scores the middle range against the topic. Returns `FitStats { block_retention, char_retention }`. 4 unit tests. |
