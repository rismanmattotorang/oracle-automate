...
$EDITOR ./.oracle-automate/destinations/dev-s4.toml   # set base_url / client / [auth]
chmod 600 ./.oracle-automate/destinations/dev-s4.toml  # the server warns if group/other-readable
```
...
./target/release/oracle-automate-server   # logs: "OData v4 backend active … target=tenant"
...
gate** stay served by Oracle-Automate's curated catalogue: a state-modifying or
...
```bash
SAP_RFC_HTTP_URL=… SAP_RFC_USER=… SAP_RFC_PASSWORD=… \
  ./target/release/oracle-automate-server --enable-writes
```
...
`sap_pool_in_use`. Import `deploy/grafana/oracle-automate-overview.json` into
