...
│   ├── oracle-automate-kb/          # KB: schema + InMemory (P1), Qdrant/Postgres (P1A)
│   ├── oracle-automate-rag/         # RAG: L2 hybrid (P3), L3 GraphRAG / L4 HippoRAG / L5 RAPTOR (P5A)
│   ├── oracle-automate-connectors/  # ADT (ABAP) + Signavio + LeanIX (P2)
│   ├── oracle-automate-skills/      # agentskills.io-compatible loader (P8A)
│   ├── oracle-automate-memory/      # working/episodic/semantic/procedural (P8)
│   └── oracle-automate-ingest/      # crawler + chunker + embedder + pipeline (P1A)
├── apps/
│   ├── oracle-automate-server/      # main MCP server binary
│   ├── oracle-automate-ingest/      # ingestion CLI (P1A)
...
| **P5A** | 20–24 (∥) | ArangoDB graph + Leiden communities + Personalised PageRank | multi-hop traversal < 400 ms P95 (≤4 hops) | **✅ done — P95 = 0.082 ms (~5000× under gate)**. `oracle-automate-graph` crate: 25-node demo graph, single-pass Louvain, PPR (α=0.15), 3-level RAPTOR. 4 new MCP tools (`kb.multi_hop`, `kb.global_query`, `kb.summarise`, `kb.graph_neighborhood`). Web Graph Lab route. |
...
- **`oracle-automate-server` binary**: exposes `abap.search`,
...
- **`oracle-automate-ingest` CLI** runs the full flow end-to-end with a
...
1. Wire real ADT REST client into `oracle-automate-connectors` (paper §IV
   data-and-I/O band).
...
| Crate features | `oracle-automate-rfc/odata` feature | pulls `reqwest`; non-default so the baseline crate stays minimal |
...
- **OpenTelemetry OTLP exporter** wired into `apps/oracle-automate-server` behind the `otlp` feature flag.  Spans propagate through every layer: MCP session → tool → SAP backend → KB / graph.
...
- **SAP Build / Joule Studio** export — generate Joule-compatible skill packages from Oracle-Automate skills, opening a migration path *into* Oracle-Automate from Joule deployments.
...
- ❌ Vendor-locked deployment.  Oracle-Automate stays infrastructure-agnostic; no AWS-only or Azure-only paths.
