...
`API_OPLACCTGDOCITEMCUBE_SRV` (ACDOCA), etc.  Oracle-Automate v1.3 ships a
...
parameter, never logged by Oracle-Automate.
...
`crates/oracle-automate-rfc/src/odata.rs` (`live_business_partner_search`).
...
SAP_BUSINESS_HUB_KEY="<your-key>" \
  cargo test -p oracle-automate-rfc --features odata live_business_partner_search -- --nocapture

# Without key — skips with a printed message:
cargo test -p oracle-automate-rfc --features odata live_business_partner_search
```
...
- The sandbox is **shared infrastructure** — be polite.  Treat it like a
  free public API.
- Oracle-Automate's `reqwest::Client` is configured with a 15-second
  timeout per request.
...
- `crates/oracle-automate-adt/tests/http_integration.rs` — 17 tests
  exercising every `HttpAdtClient` URL pattern, header, CSRF flow,
  XML parser path against an axum-fixture server.
- `apps/oracle-automate-server/tests/cache_tools.rs`,
  `kb_navigate.rs`,
  `spec_utilities.rs`,
...
5. **RFC over SOAP — no NetWeaver RFC SDK needed.**  Oracle-Automate's
   ...
   SAP_RFC_HTTP_URL=… SAP_RFC_USER=… SAP_RFC_PASSWORD=… \
     cargo test -p oracle-automate-rfc --features soap live_read_table_t000 -- --nocapture
   ```
