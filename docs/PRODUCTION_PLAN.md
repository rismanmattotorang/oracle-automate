...
> *tested against a real Kalbe development S/4HANA system* — not the
...
|---|---|---|---|
| RFC / BAPI calls (`sap.rfc.call`) | "12 SAP system / RFC tools" | **`MockSapClient` only.** No real RFC transport exists in `oracle-automate-rfc`. | ❌ No |
...
| Signavio / LeanIX connectors | "ABAP + BPMN + LeanIX" | `oracle-automate-connectors` is a 40-line stub | ❌ No |

**Bottom line:** the only two pieces of real network code (`HttpAdtClient`,
`BusinessHubClient`) cannot today reach a Kalbe dev tenant — one is
...
| **`verify`** / **`run`** | End of each sprint | Actually launch `oracle-automate-server` pointed at the dev tenant and observe a real tool call returning real data. |
...
`deploy/grafana/oracle-automate-overview.json` (tool-latency P95/P99 vs the 80 ms
...
The goal is met when, against the Kalbe dev tenant:
