...
//!
//! This binary:
//!   1. Spawns the Oracle-Automate MCP server as a child process over stdio.
//!   2. Registers channel adapters (CLI now; Teams / Slack / Telegram
//!      land via the trait in `oracle-automate-channels`).
...
#[derive(Parser)]
#[command(
    name = "oracle-automate-gw",
    about = "Multi-channel gateway routing channel events into MCP tool calls."
)]
struct Cli {
    /// Path to the Oracle-Automate MCP server binary.
    #[arg(long, default_value = "target/release/oracle-automate-server")]
    server: String,
