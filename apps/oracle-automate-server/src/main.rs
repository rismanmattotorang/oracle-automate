//! Oracle-Automate MCP server binary.
//!
//! Phase 2: this binary is the upgraded Oracle-Automate server, incorporating
...
use oracle_automate_server_lib::{context, prompts, resources, seed, tools};
...
async fn main() -> anyhow::Result<()> {
    ...
    let skill_paths: Vec<std::path::PathBuf> = vec![
        ...
        std::path::PathBuf::from("./.oracle-automate/skills"),
        dirs_config_path("oracle-automate/skills"),
    ];
    ...
    tracing::info!(
        ...
        "Oracle-Automate server configured"
    );
...
fn build_server(ctx: Arc<ServerContext>, agents_md: &Option<String>, read_only: bool, skills: &SkillRegistry) -> Server {
    ...
    let mut builder = Server::builder("oracle-automate-server", env!("CARGO_PKG_VERSION"))
        .exposure(policy)
