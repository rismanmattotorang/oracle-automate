...
use oracle_automate_ingest::{EmbeddingClient, HelpPortalCrawler, IngestionPipeline, MockEmbedder};
use oracle_automate_kb::{InMemoryKb, KnowledgeStore};
use oracle_automate_rag::{GraphEngine, MockReranker, Query, RagEngine};
use std::sync::Arc;
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut chunker_cfg = oracle_automate_ingest::ChunkerConfig::default();
    ...
    let mut layer_samples: Vec<oracle_automate_rag::LatencyBreakdown> = Vec::with_capacity(cli.n);
    ...
    let layer_mean = |f: fn(&oracle_automate_rag::LatencyBreakdown) -> u64| -> u64 {
