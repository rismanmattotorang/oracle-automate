...
fn draw_latency_gauge(f: &mut Frame, app: &App, area: Rect) {
    ...
    let gauge = Gauge::default()
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(" P95 latency budget (80 ms gate) "),
        )
...
fn draw_per_tool_breakdown(f: &mut Frame, app: &App, area: Rect) {
    let mut rows: Vec<Row> = app
        ...
        .map(|t| {
            ...
            Row::new(vec![
                ...
                Cell::from(format!("{:>5}", b.total_us)).style(if b.total_us > 80_000 {
                    Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)
                } else if b.total_us > 40_000 {
                    Style::default().fg(Color::Yellow)
                } else {
                    Style::default().fg(Color::Green)
                }),
    ...
    if rows.is_empty() {
        rows.push(Row::new(vec![
            Cell::from("(no RAG tool traffic yet)").style(Style::default().fg(Color::DarkGray))
        ]));
    }
    let table = Table::new(
        rows,
    ...
    .block(
        Block::default()
            .borders(Borders::ALL)
            .title(" RAG pipeline — per-tool layer breakdown "),
    );
