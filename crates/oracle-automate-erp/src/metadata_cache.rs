...
/// Decorator that caches `ErpOperationMeta` keyed by `(function, language)`
...
impl<C: ErpClient + ?Sized> MetadataCache<C> {
    ...
    async fn get_fresh(&self, key: &(String, String)) -> Option<ErpOperationMeta> {
    ...
    async fn store(&self, key: (String, String), meta: ErpOperationMeta) {
        if self.ttl.is_zero() {
            return;
        }
        let mut entries = self.entries.write().await;
...
impl<C: ErpClient + ?Sized> ErpClient for MetadataCache<C> {
    ...
    async fn operation_metadata(&self, function: &str, language: &str) -> ErpResult<ErpOperationMeta> {
        ...
        if self.ttl.is_zero() {
            ...
            return self.inner.operation_metadata(function, language).await;
        }
        ...
        self.stats.write().await.misses += 1;
        let meta = self.inner.operation_metadata(function, language).await?;
