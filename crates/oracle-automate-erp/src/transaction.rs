...
pub async fn execute_write_bapi(
    ...
    if request.function == COMMIT_FN || request.function == ROLLBACK_FN {
        return Err(ErpError::InvalidParameter {
            name: "function".into(),
            reason:
                "commit/rollback are issued automatically; call the business REST operation instead"
                    .into(),
        });
    }
...
mod tests {
    ...
    fn has_failure_treats_error_abort_unknown_as_failure() {
        ...
        assert!(!has_failure(&[
            msg(ErpSeverity::Warning),
            msg(ErpSeverity::Info)
        ]));
        assert!(!has_failure(&[]));
    }
    ...
    async fn refuses_in_read_only_mode() {
        ...
        let err = execute_write_bapi(client.as_ref(), req, true)
            .await
            .unwrap_err();
    ...
    async fn rejects_direct_commit_call() {
        ...
        let err = execute_write_bapi(client.as_ref(), req, false)
            .await
            .unwrap_err();
        assert!(
            matches!(err, ErpError::InvalidParameter { .. }),
            "got {err:?}"
        );
    }
    ...
    async fn empty_bapiret2_is_fail_closed_not_committed() {
        ...
        let outcome = execute_write_bapi(client.as_ref(), req, false)
            .await
            .unwrap();
    ...
    impl ErpClient for ScriptedClient {
        ...
        async fn search_operations(
            ...
            _q: &str,
            _n: usize,
        ) -> ErpResult<crate::client::ErpSearchResult> {
            Err(ErpError::Internal("unused".into()))
        }
        async fn operation_metadata(
            ...
            _f: &str,
            _l: &str,
        ) -> ErpResult<crate::client::ErpOperationMeta> {
            Err(ErpError::Internal("unused".into()))
        }
        async fn bulk_operation_metadata(
            ...
            _f: &[String],
            _l: &str,
        ) -> ErpResult<crate::client::BulkMetadata> {
            Err(ErpError::Internal("unused".into()))
        }
        async fn read_table(
            &self,
            _r: crate::client::ReadTableRequest,
        ) -> ErpResult<Vec<crate::client::TableRow>> {
            Err(ErpError::Internal("unused".into()))
        }
    ...
    async fn commits_on_explicit_success_row() {
        let client = scripted(
            json!([{ "TYPE": "S", "ID": "06", "NUMBER": "017", "MESSAGE": "PO 4500000001 created" }]),
        );
        let req = ErpCallRequest {
            function: "fusion.po.purchaseOrders.post".into(),
    ...
    async fn rolls_back_on_error_row() {
        let client = scripted(
            json!([{ "TYPE": "E", "ID": "06", "NUMBER": "055", "MESSAGE": "Vendor 1 blocked" }]),
        );
        let req = ErpCallRequest {
            function: "fusion.po.purchaseOrders.post".into(),
            parameters: json!({ "POHEADER": {} }),
